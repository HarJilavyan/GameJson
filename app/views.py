    ...
    def post(self):
        ...
        if new_user_rights =="":
            return self.json_response(
                {
                    "status": "Fail",
                    "message": "Please choose rights",
                    "field": "rights"
                })
        hashed_password = bcrypt.hashpw(new_user_password.encode(), bcrypt.gensalt())
        users.sign_up(new_user_name, hashed_password, new_user_rights)
        user = users.login(new_user_name, hashed_password)
        self.principal = Principal(id=str(user[0]), roles=str(new_user_rights))
